...
object ProcessInstance {
  ...
  def props[P : Identifiable, T : Identifiable, S, E](processType: String, petriNet: PetriNet[P, T], petriNetRuntime: ProcessInstanceRuntime[P, T, S, E], settings: Settings): Props =
...
class ProcessInstance[P : Identifiable, T : Identifiable, S, E](
  ...
  private implicit def fromExecutionInstance(instance: com.ing.baker.runtime.actor.process_instance.internal.Instance[P, T, S]): InstanceState =
  ...
  private implicit def fromExecutionJob(job: com.ing.baker.runtime.actor.process_instance.internal.Job[P, T, S]): JobState =
    JobState(job.id, job.transition.getId, job.consume.marshall, job.input, job.failure.map(fromExecutionExceptionState))

  private implicit def fromExecutionExceptionState(exceptionState: com.ing.baker.runtime.actor.process_instance.internal.ExceptionState): ExceptionState =
  ...
  private implicit def fromExecutionExceptionStrategy(strategy: com.ing.baker.runtime.actor.process_instance.internal.ExceptionStrategy): ExceptionStrategy = strategy match {
    case internal.ExceptionStrategy.Fatal => ExceptionStrategy.Fatal
    case internal.ExceptionStrategy.BlockTransition => ExceptionStrategy.BlockTransition
    case com.ing.baker.runtime.actor.process_instance.internal.ExceptionStrategy.RetryWithDelay(delay) => ExceptionStrategy.RetryWithDelay(delay)
    case com.ing.baker.runtime.actor.process_instance.internal.ExceptionStrategy.Continue(marking, output) => ExceptionStrategy.Continue(marking.asInstanceOf[Marking[P]].marshall, output)
  }
  ...
  def scheduleFailedJobsForRetry(instance: Instance[P, T, S]): Map[Long, Cancellable] = {
    instance.jobs.values.foldLeft(Map.empty[Long, Cancellable]) {
      case (map, j @ Job(_, _, _, _, _, _, Some(com.ing.baker.runtime.actor.process_instance.internal.ExceptionState(failureTime, _, _, RetryWithDelay(delay))))) ⇒
