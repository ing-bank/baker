...
import com.ing.baker.il.{EventType, getRawClass}
...
object InteractionImplementation {
  ...
  def anyToInteractionImplementations(any: AnyRef) : Seq[InteractionImplementation] = {
    ...
    getPossibleInteractionNamesForImplementation(any).map { name =>
      new InteractionImplementation(name, any, applyMethod.getParameterTypes.toSeq, applyMethod.getReturnType)
    }.toSeq
  }
}

case class InteractionImplementation(name: String,
                                     implementation: AnyRef,
                                     requiredIngredients: Seq[Type],
                                     returnType: Type) {

  val log: Logger = LoggerFactory.getLogger(InteractionImplementation.getClass)
  ...
  private val interactionTask: Seq[Any] => Any = inputArgs => {
    ...
    val method = implementation.getClass.getMethod("apply", requiredIngredients.map { getRawClass }: _*)
