  ...
  "Baker" should {
    ...
    "notify ProcessCreated/EventReceived/InteractionStarted/InteractionCompleted events with correct timestamps" in {
      ...
      for {
        ...
        _ <- baker.fireEventAndResolveWhenCompleted(processId, EventInstance.unsafeFrom(InitialEvent(initialIngredientValue)), "someId")
        ...
        _ = expectMsgInAnyOrderPF(listenerProbe,
          ...
          { case msg@EventReceived(_, _, _, `processId`, Some("someId"), EventInstance("InitialEvent", ingredients)) if ingredients == Map("initialIngredient" -> PrimitiveValue(`initialIngredientValue`)) => msg },
          ...
          { case msg@InteractionCompleted(_, _, _, _, `processId`, "InteractionOne", Some(EventInstance("InteractionOneSuccessful", ingredients))) if ingredients == Map("interactionOneIngredient" -> PrimitiveValue("interactionOneIngredient")) => msg },
          { case msg@InteractionCompleted(_, _, _, _, `processId`, "InteractionTwo", Some(EventInstance("EventFromInteractionTwo", ingredients))) if ingredients == Map("interactionTwoIngredient" -> PrimitiveValue("interactionTwoIngredient")) => msg },
          { case msg@InteractionCompleted(_, _, _, _, `processId`, "InteractionThree", Some(EventInstance("InteractionThreeSuccessful", ingredients))) if ingredients == Map("interactionThreeIngredient" -> PrimitiveValue("interactionThreeIngredient")) => msg },
          { case msg@InteractionCompleted(_, _, _, _, `processId`, "ProvidesNothingInteraction", None) => msg },
          { case msg@InteractionCompleted(_, _, _, _, `processId`, "SieveInteraction", Some(EventInstance("SieveInteractionSuccessful", ingredients))) if ingredients == Map("sievedIngredient" -> PrimitiveValue("sievedIngredient")) => msg }
        )
    ...
    "notify EventRejected event with InvalidEvent reason" in {
      ...
      for {
        ...
        _ = baker.fireEventAndResolveWhenReceived(processId, EventInstance.unsafeFrom(ThirdEvent()), "someId")
        _ = listenerProbe.fishForSpecificMessage(eventReceiveTimeout) {
          case msg@EventRejected(_, `processId`, Some("someId"), EventInstance("ThirdEvent", ingredients), InvalidEvent) if ingredients.isEmpty => msg
        }
        _ = listenerProbe.expectNoMessage(eventReceiveTimeout)
      } yield succeed
    }
    ...
    "notify EventRejected event with AlreadyReceived reason" in {
      ...
      for {
        ...
        _ <- baker.bake(recipeId, processId)
        _ <- baker.fireEventAndResolveWhenCompleted(processId, EventInstance.unsafeFrom(InitialEvent(initialIngredientValue)), "someId")
        _ <- baker.fireEventAndResolveWhenCompleted(processId, EventInstance.unsafeFrom(InitialEvent(initialIngredientValue)), "someId") // Same correlationId cannot be used twice
        _ = listenerProbe.fishForSpecificMessage(eventReceiveTimeout) {
          case msg@EventRejected(_, `processId`, Some("someId"), EventInstance("InitialEvent", ingredients), AlreadyReceived) if ingredients == Map("initialIngredient" -> PrimitiveValue(`initialIngredientValue`)) => msg
        }
        _ = listenerProbe.expectNoMessage(eventReceiveTimeout)
      } yield succeed
    }
    ...
    "notify EventRejected event with FiringLimitMet reason" in {
      ...
      for {
        ...
        _ <- baker.fireEventAndResolveWhenCompleted(processId, EventInstance.unsafeFrom(InitialEvent(initialIngredientValue))) // Firing limit is set to 1 in the recipe
        _ = listenerProbe.fishForSpecificMessage(eventReceiveTimeout) {
          case msg@EventRejected(_, `processId`, None, EventInstance("InitialEvent", ingredients), FiringLimitMet) if ingredients == Map("initialIngredient" -> PrimitiveValue(`initialIngredientValue`)) => msg
        }
        _ = listenerProbe.expectNoMessage(eventReceiveTimeout)
      } yield succeed
    }
    ...
    "notify EventRejected event with ReceivePeriodExpired reason" in {
      ...
      for {
        ...
        _ = baker.fireEventAndResolveWhenReceived(processId, EventInstance.unsafeFrom(InitialEvent(initialIngredientValue)), "someId")
        _ = listenerProbe.fishForSpecificMessage(eventReceiveTimeout) {
          case msg@EventRejected(_, `processId`, Some("someId"), EventInstance("InitialEvent", ingredients), ReceivePeriodExpired) if ingredients == Map("initialIngredient" -> PrimitiveValue(`initialIngredientValue`)) => msg
        }
        _ = listenerProbe.expectNoMessage(eventReceiveTimeout)
      } yield succeed
    }
    ...
    "notify EventRejected event with NoSuchProcess reason" in {
      ...
      for {
        ...
        _ = baker.fireEventAndResolveWhenReceived(processId, EventInstance.unsafeFrom(InitialEvent(initialIngredientValue)), "someId")
        _ = listenerProbe.fishForSpecificMessage(eventReceiveTimeout) {
          case msg@EventRejected(_, `processId`, Some("someId"), EventInstance("InitialEvent", ingredients), NoSuchProcess) if ingredients == Map("initialIngredient" -> PrimitiveValue(`initialIngredientValue`)) => msg
        }
        _ = listenerProbe.expectNoMessage(eventReceiveTimeout)
      } yield succeed
    }
