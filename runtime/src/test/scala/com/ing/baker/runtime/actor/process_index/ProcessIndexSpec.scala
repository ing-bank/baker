    ...
    override def receive: Receive = {
      case GetAllRecipes => {
        sender ! AllRecipes(Seq[RecipeInformation](
          RecipeInformation(recipeId, CompiledRecipe("name", recipeId, new PetriNet(Graph.empty), Marking.empty, Seq.empty, Option.empty, Option.empty), 0L)))
      }
    }
  }))
  ...
  "ProcessIndex" should {
    ...
    "delete a process if a retention period is defined, stop command is received" in {
      ...
      recipeManagerProbe.reply(AllRecipes(Seq[RecipeInformation](
        ...
        CompiledRecipe("name", recipeId, new PetriNet(Graph.empty), Marking.empty, Seq.empty, Option.empty, Some(recipeRetentionPeriod)),
          0L))))
    ...
    "Forward the FireTransition command when a valid HandleEvent is sent" in {
      ...
      recipeManagerProbe.reply(AllRecipes(Seq[RecipeInformation](
        RecipeManagerProtocol.RecipeInformation(recipeId,
          CompiledRecipe("name", recipeId, petrinetMock, Marking.empty, Seq.empty, Option.empty, Option.empty),
          0L))))
    ...
    "reply with a InvalidEvent message when attempting to fire an event that is now know in the compiledRecipe" in {
      ...
      recipeManagerProbe.reply(AllRecipes(Seq[RecipeInformation](
        RecipeManagerProtocol.RecipeInformation(recipeId,
          CompiledRecipe("name", recipeId, new PetriNet(Graph.empty), Marking.empty, Seq.empty, Some(receivePeriodTimeout), Option.empty),
          0L))))
    ...
    "reply with a InvalidEvent message when attempting to fire an event that does not comply to the recipe" in {
      ...
      recipeManagerProbe.reply(AllRecipes(Seq[RecipeInformation](
        RecipeManagerProtocol.RecipeInformation(recipeId,
          CompiledRecipe("name", recipeId, petrinetMock, Marking.empty, Seq.empty, Some(receivePeriodTimeout), Option.empty),
          0L))))
