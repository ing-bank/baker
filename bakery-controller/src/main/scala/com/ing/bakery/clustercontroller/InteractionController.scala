...
object InteractionController extends LazyLogging {
  ...
  def resource(k8s: KubernetesClient)(implicit contextShift: ContextShift[IO], timer: Timer[IO], actorSystem: ActorSystem, materializer: Materializer): Resource[IO, Unit] = {
    ...
    def handleEvent(event: K8SWatchEvent[InteractionResource]): IO[Unit] = {
      val ops = new InteractionOps(event._object, k8s)
      println(Console.YELLOW + event._type + " " + event._object.metadata.name + Console.RESET)
      event._type match {
        case EventType.ADDED => ops.createInteraction
    ...
    val create = for {
      killSwitch <- IO {
        k8s.watchAllContinuously[InteractionResource]()
