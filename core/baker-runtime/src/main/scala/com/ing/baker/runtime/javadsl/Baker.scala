...
class Baker(private val baker: scaladsl.Baker) extends BakerCommon {
  ...
  override def close(): Unit =
    toCompletableFuture(baker.gracefulShutdown()).orTimeout(10, TimeUnit.SECONDS).join()
   ...
   */
  def gracefulShutdown(): CompletableFuture[Void] =
    toCompletableFutureVoid(baker.gracefulShutdown())
   ...
   */
  def bake(@Nonnull recipeId: String, @Nonnull recipeInstanceId: String): CompletableFuture[Void] =
    toCompletableFutureVoid(baker.bake(recipeId, recipeInstanceId))
   ...
   * @return
   */
  def bake(recipeId: String, recipeInstanceId: String, metadata: util.Map[String, String]): CompletableFuture[Void] =
    toCompletableFutureVoid(baker.bake(recipeId, recipeInstanceId, metadata.asScala.toMap))

  def deleteRecipeInstance(recipeInstanceId: String, removeFromIndex: Boolean): CompletableFuture[Void] =
    toCompletableFutureVoid(baker.deleteRecipeInstance(recipeInstanceId, removeFromIndex))
   ...
   * @return
   */
  def retryInteraction(@Nonnull recipeInstanceId: String, @Nonnull interactionName: String): CompletableFuture[Void] =
    toCompletableFutureVoid(baker.retryInteraction(recipeInstanceId, interactionName))
   ...
   * @return
   */
  def resolveInteraction(@Nonnull recipeInstanceId: String, @Nonnull interactionName: String, @Nonnull event: EventInstance): CompletableFuture[Void] =
    toCompletableFutureVoid(baker.resolveInteraction(recipeInstanceId, interactionName, event.asScala))
   ...
   * @return
   */
  def stopRetryingInteraction(@Nonnull recipeInstanceId: String, @Nonnull interactionName: String): CompletableFuture[Void] =
    toCompletableFutureVoid(baker.stopRetryingInteraction(recipeInstanceId, interactionName))
  ...
  override def getRecipeVisual(recipeId: String, style: RecipeVisualStyle): CompletableFuture[String] =
   ...
   */
  override def registerEventListener(@Nonnull recipeName: String, @Nonnull listenerFunction: BiConsumer[RecipeEventMetadata, String]): CompletableFuture[Void] =
    toCompletableFutureVoid(baker.registerEventListener(recipeName,
   ...
   */
  override def registerEventListener(@Nonnull listenerFunction: BiConsumer[RecipeEventMetadata, String]): CompletableFuture[Void] =
    toCompletableFutureVoid(baker.registerEventListener(
      (recipeEventMetadata: scaladsl.RecipeEventMetadata, event: String) => listenerFunction.accept(recipeEventMetadata.asJava, event)))
  ...
  def registerEventListener(@Nonnull eventListener: EventListener): CompletableFuture[Void] =
   ...
   *
   * @param listenerFunction Listener function that is called once these events occur
   * @return
   */
  override def registerBakerEventListener(@Nonnull listenerFunction: Consumer[BakerEvent]): CompletableFuture[Void] =
    toCompletableFutureVoid(baker.registerBakerEventListener((event: scaladsl.BakerEvent) => listenerFunction.accept(event.asJava)))
  ...
  @nowarn
  private def toCompletableFutureVoid(@Nonnull scalaFuture: Future[Unit]): CompletableFuture[Void] =
    scalaFuture.asJava.toCompletableFuture.thenApply((_: Unit) => null.asInstanceOf[Void])
   ...
   * @param metadata Metadata
   */
  override def addMetaData(recipeInstanceId: String, metadata: java.util.Map[String, String]): CompletableFuture[Void] = {
    val x: Map[String, String] = metadata.asScala.toMap
    toCompletableFutureVoid(baker.addMetaData(recipeInstanceId, x))
  }
  ...
  override def awaitEvent(recipeInstanceId: String, eventName: String, timeout: Duration, waitForNext: Boolean): CompletableFuture[Void] =
    toCompletableFutureVoid(
      baker.awaitEvent(recipeInstanceId, eventName, FiniteDuration.apply(timeout.toMillis, TimeUnit.MILLISECONDS), waitForNext)
    )
