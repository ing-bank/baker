...
class InMemoryEventStream : EventStream<InMemoryEffect<*>> {

    private val store: ConcurrentLinkedQueue<Function1<BakerEvent, BoxedUnit>> = ConcurrentLinkedQueue()

    override fun fetchListeners(): InMemoryEffect<ScalaList<Function1<BakerEvent, BoxedUnit>>> =
        InMemoryEffects.pure(
            ScalaList.from(CollectionHasAsScala(store).asScala()) as ScalaList<Function1<BakerEvent, BoxedUnit>>
        )

    override fun subscribe(listenerFunction: Function1<BakerEvent, BoxedUnit>): InMemoryEffect<BoxedUnit> =
        InMemoryEffects.pure(store.add(listenerFunction)).map { BoxedUnit.UNIT }
}
