...
private object RecipeCompilerScala {
  ...
  private def buildInteractionArcs(multipleOutputFacilitatorTransitions: Seq[Transition],
    ...
    val inputArcs: Seq[Arc] = buildInteractionInputArcs(t, multipleOutputFacilitatorTransitions, placeNameWithDuplicateTransitions)
  ...
  def compileRecipe(recipe: Recipe,
    ...
    val precompileErrors: Seq[String] = PreCompileValidations.INSTANCE.preCompileAssertions(recipe)
  ...
  private def getMultiTransition(internalRepresentationName: String, transitions: Seq[Transition]): Transition =
    transitions
  ...
  private def convertCheckpointEventToInteraction(e: CheckPointEvent) =
    Interaction(
  ...
  private def convertSieveToInteraction(s: Sieve) =
    Interaction(
  ...
  private def flattenSubRecipesToInteraction(recipe: Recipe): Set[InteractionDescriptor] = {
  ...
  private def flattenSensoryEvents(recipe: Recipe): Set[com.ing.baker.recipe.common.Event] = {
