...
object ScalaConversions {
    ...
    val <T> List<T>.asScala get(): Seq<T> = ListHasAsScala(toList()).asScala().toSeq()
    val <T> Set<T>.asScala get(): scala.collection.immutable.Set<T> = SetHasAsScala(this).asScala().toSet()
    val <K, V> Map<K, V>.asScala
        get(): scala.collection.immutable.Map<K, V> =
            MapHasAsScala(this).asScala().toMap(
