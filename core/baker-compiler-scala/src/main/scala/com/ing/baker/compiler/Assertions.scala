...
object Assertions {
  ...
  private def assertRequiredEventForReprovider(recipe: Recipe): Seq[String] = {
    ...
    }).map(interaction => s"Reprovider interaction ${interaction.name} needs to have a event precondition")
  }
