 ...
 */
typealias RecipePetriNet = PetriNet
...
object RecipeCompilerKotlin {
        ...
        interaction.requiredEvents.map { eventName ->
            ...
            val eventPreconditionPlace =
                createPlace(
                    ...
                    placeType = Place.`EventPreconditionPlace$`.`MODULE$`
                )
    ...
    private fun buildEventPreconditionArcs(eventName: String,
                                           preconditionPlace: Place,
                                           preconditionTransition: (String) -> Transition?,
                                           interactionTransition: Transition
    ): Pair<List<Arc>, List<String>> {

        val eventTransition: Transition? = preconditionTransition(eventName)

        val notProvidedError: List<String> = when(eventTransition) {
    ...
    private fun buildInteractionOutputArcs(
        ...
        if (interaction.eventsToFire.isNotEmpty()) {
            val resultPlace = createPlace(label = interaction.label(), placeType = Place.`InteractionEventOutputPlace$`.`MODULE$`)
            val eventArcs = interaction.eventsToFire.flatMap { event: EventDescriptor ->
                //Get the correct event transition
                val eventTransition = eventTransitions.find {it.event().name() == event.name() } ?: throw IllegalStateException("eventTransition should be found")
                ...
                if(eventTransitionCount > 1) {
                    ...
                    val eventCombinerPlace: Place = createPlace(label = event.name(), placeType = Place.`IntermediatePlace$`.`MODULE$`)
                    //Create a new intermediate event transition
                    val interactionToEventTransition: IntermediateTransition = IntermediateTransition("${interaction.interactionName()}:${event.name()}")
                    //link the interaction output place to the intermediate transition
                    val interactionOutputPlaceToIntermediateTransition: Arc = arc(resultPlace, interactionToEventTransition, 1, event.name())
                    ...
                    listOf(intermediateTransitionToEventCombinerPlace, interactionOutputPlaceToIntermediateTransition, eventCombinerPlaceToEventTransition)
                }
                else {
                    val internalEventTransition = eventTransition
                    listOf(arc(resultPlace, internalEventTransition, 1, event.name()))
                }
            }
    ...
    private fun buildInteractionInputArcs(
        ...
        ingredientsWithMultipleConsumers: Map<String, List<InteractionTransition>>): List<Arc> {

        val (fieldNamesWithPrefixMulti, fieldNamesWithoutPrefix) =
            t.nonProvidedIngredients.map(IngredientDescriptor::name).partition(ingredientsWithMultipleConsumers::contains)
        ...
        val internalDataInputArcs = fieldNamesWithPrefixMulti.flatMap { fieldName ->
            val multiTransitionPlace = createPlace("${t.label()}-$fieldName", placeType = Place.`MultiTransitionPlace$`.`MODULE$`)
            listOf(
        ...
        val dataInputArcs: List<Arc> = fieldNamesWithoutPrefix.map { fieldName -> arc(createPlace(fieldName, Place.`IngredientPlace$`.`MODULE$`), t, 1) }

        val dataOutputArcs: List<Arc> =
        if (t.isReprovider)
            fieldNamesWithoutPrefix.map { fieldName -> arc(t, createPlace(fieldName, Place.`IngredientPlace$`.`MODULE$`), 1) } +
            fieldNamesWithPrefixMulti.map { fieldName -> arc(t, createPlace("${t.label()}-$fieldName", placeType = Place.`MultiTransitionPlace$`.`MODULE$`), 1) }
        else
            emptyList()

        val limitInteractionCountArc: List<Arc> = when (val maximumInteractionCount = t.maximumInteractionCount) {
            null -> emptyList()
            else -> listOf(arc(createPlace("limit:${t.label()}", Place.FiringLimiterPlace(maximumInteractionCount)), t, 1))
        }
    ...
    fun compileRecipe(
        ...
        val actionDescriptors: List<InteractionDescriptor> = recipe.interactions +
                ...
                recipe.sieves.map(::convertSieveToInteraction)//checkPointSieveSubRecipeInteractions(recipe)

        fun subRecipeSensoryEvents(recipe: Recipe): Set<Event> =
            recipe.subRecipes.flatMap(com.ing.baker.recipe.common.Recipe::sensoryEvents).toSet<Event>()

        //All ingredient names provided by sensory events or by interactions
        val allIngredientNames: Set<String> =
            (recipe.sensoryEvents +
                    subRecipeSensoryEvents(recipe)).flatMap { e -> e.providedIngredients.map(Ingredient::name) }
                .toSet() +
        ...
        val interactionTransitions =
            actionDescriptors.map { interactionTransitionOf(it, recipe.defaultFailureStrategy(), allIngredientNames) }

        val allInteractionTransitions: List<InteractionTransition> = interactionTransitions

        // events provided from outside
        //TODO: handle sensoryEvents subrecipe Recursion
        val sensoryEventTransitions: List<EventTransition> = (recipe.sensoryEvents + recipe.subRecipes.flatMap {it.sensoryEvents }).map {
            event -> EventTransition(eventToCompiledEvent(event), true, event.maxFiringLimit())
        }
        ...
        val internalEventArcs: List<Arc> = allInteractionTransitions.flatMap { t ->
            t.eventsToFire.flatMap { event ->
                event.ingredients.map {
                    ingredient ->
                    val from = interactionEventTransitions.find { it.label() == event.name() } ?: throw IllegalStateException()
                    arc(from, createPlace(ingredient.name(), Place.`IngredientPlace$`.`MODULE$`), 1)
                }
            }
        }

        //Create event limiter places so that events can only fire x amount of times.
        val eventLimiterArcs: List<Arc> = sensoryEventTransitions.flatMap { t -> when(val n = t.maxFiringLimit().getOrElse {null as Int?}) {
            null -> emptyList()
            else -> listOf(arc(createPlace("limit:${t.label()}", Place.FiringLimiterPlace(n)), t, 1))
        }}


        fun findEventTransitionByEventName(eventName: String) = allEventTransitions.find { it.event().name() == eventName }

        fun findInteractionByLabel  (label: String) = allInteractionTransitions.find { it.label() == label } ?: throw IllegalStateException()

        // This generates precondition arcs for Required Events (AND).
        val (eventPreconditionArcs, preconditionANDErrors) = actionDescriptors.map {
            t ->
        ...
        val (eventOrPreconditionArcs, preconditionORErrors) = actionDescriptors.map {
            t ->
            buildEventORPreconditionArcs(
                t,
                ::findEventTransitionByEventName,
                ::findInteractionByLabel
            )
        ...
        val sensoryEventArcs: List<Arc> = sensoryEventWithIngredients.flatMap { et ->
            et.event().ingredients.map { ingredient ->
                arc(et, createPlace(ingredient.name(), Place.`IngredientPlace$`.`MODULE$`), 1)
            }
        }

        val eventThatArePreconditions: List<String> = actionDescriptors.flatMap {
        ...
        .map { sensoryEvent -> arc(sensoryEvent, createPlace(sensoryEvent.label(), Place.`EmptyEventIngredientPlace$`.`MODULE$`), 1) }
        ...
        val multipleOutputFacilitatorArcs: List<Arc> =
            multipleConsumerFacilitatorTransitions.map { t ->
                arc(createPlace(t.label(), Place.`IngredientPlace$`.`MODULE$`), t, 1) }
        ...
        val petriNet: PetriNet = PetriNet(graph(arcs))

        val initialMarking: Marking<Place> = petriNet.places().asJava.mapNotNull { p ->
            when (val placeType = p.placeType()) {
                is FiringLimiterPlace ->
                    p to mapOf<Any?, Int>(null to placeType.maxLimit())
        ...
        val oldRecipeIdVariant: OldRecipeIdVariant =
            when (recipe) {
                is com.ing.baker.recipe.javadsl.Recipe -> CompiledRecipe.`Scala212CompatibleJava$`.`MODULE$`
                is com.ing.baker.recipe.kotlindsl.Recipe -> CompiledRecipe.`Scala212CompatibleKotlin$`.`MODULE$`
                is com.ing.baker.recipe.scaladsl.Recipe -> CompiledRecipe.`Scala212CompatibleScala$`.`MODULE$`
                else -> throw IllegalArgumentException("Only recipe of type java, kotlin or scala supported")
            }
    ...
    fun compileRecipe(recipe: Recipe): CompiledRecipe = compileRecipe(recipe, `ValidationSettings$`.`MODULE$`.defaultValidationSettings())
    ...
    private fun getIngredientsWithMultipleConsumers(actionTransitions: List<InteractionTransition>): Map<String, List<InteractionTransition>> =
        ...
        actionTransitions.flatMap { transition ->
    ...
    private fun convertCheckpointEventToInteraction(e: com.ing.baker.recipe.common.CheckPointEvent) =
        interaction(
            name = "${com.ing.baker.il.`package$`.`MODULE$`.checkpointEventInteractionPrefix()}${e.name()}",
            inputIngredients = emptyList(),
            output = listOf(`Event$`.`MODULE$`.apply(e.name(), emptyList<Ingredient>().asScala)),
            requiredEvents = e.requiredEvents,
            requiredOneOfEvents = e.requiredOneOfEvents
        )

    private fun convertSieveToInteraction(s: com.ing.baker.recipe.common.Sieve) =
        interaction(
            name = "${com.ing.baker.il.`package$`.`MODULE$`.sieveInteractionPrefix()}${s.name()}",
    ...
    private fun flattenSubRecipesToInteraction(recipe: com.ing.baker.recipe.common.Recipe): Set<InteractionDescriptor> {
        fun copyInteraction(i: InteractionDescriptor) = com.ing.baker.recipe.scaladsl.Interaction.apply(
            "${com.ing.baker.il.`package$`.`MODULE$`.subRecipePrefix()}${recipe.name()}$$${i.name()}",
            i.inputIngredients(),
            i.output(),
            i.requiredEvents(),
        ...
        return recipe.interactions.map(::copyInteraction).toSet() + checkPointSieveSubRecipeInteractions(recipe)
    }

        private fun checkPointSieveSubRecipeInteractions(recipe: com.ing.baker.recipe.common.Recipe): List<InteractionDescriptor> =
        recipe.checkpointEvents.map(::convertCheckpointEventToInteraction).toList() +
                recipe.subRecipes.flatMap(::flattenSubRecipesToInteraction) +
                recipe.sieves.map(::convertSieveToInteraction).toList()



}

fun ingredientToCompiledIngredient(ingredient: Ingredient): IngredientDescriptor = IngredientDescriptor(ingredient.name(), ingredient.ingredientType())

fun eventToCompiledEvent(event: Event): EventDescriptor = EventDescriptor(event.name(), event.providedIngredients.map(::ingredientToCompiledIngredient).asScala)


private fun graph(arcs: List<Arc>): Graph<Node, WLDiEdge<Any>> {
    val defaultHints = object : Hints {
        override fun initialCapacity() = 16
        override fun capacityIncrement() = 32
        override fun hashTableThreshold() = 48
        override fun compactUpToUsed() = 80
    }

    return `Graph$`.`MODULE$`.from(arcs.asScala, ClassTag.apply(WLDiEdge::class.java), CoreConfig(
        GraphConfig.defaultOrder(), defaultHints
    )) as Graph<Node, WLDiEdge<Any>>
}


private fun <N, L> wlDiEdge(
    node1: Node,
    node2: Node,
    weight: Double,
    label: L
): WLDiEdge<N> =
    `WLDiEdge$`.`MODULE$`.newEdge(
        GraphEdge.`NodeProduct$`.`MODULE$`.apply(node1, node2),
        weight,
        label
    )
...
fun interactionTransitionOf(
    ...
    fun transformEventType(event: Event): Event =
        when (val eventOutputTransformer = interactionDescriptor.eventOutputTransformers.get(event)) {
            null -> event
            else -> com.ing.baker.recipe.scaladsl.Event(
                eventOutputTransformer.newEventName,
                event.providedIngredients.map { i ->
                    Ingredient(eventOutputTransformer.ingredientRenames.getOrElse(i.name(), { i.name() }), i.ingredientType())}.asScala,
                Option.empty<Any>()
            )
        }
    ...
    fun transformEventOutputTransformer(recipeEventOutputTransformer: EventOutputTransformer): com.ing.baker.il.EventOutputTransformer =
        com.ing.baker.il.EventOutputTransformer(recipeEventOutputTransformer.newEventName(), recipeEventOutputTransformer.ingredientRenames())

    fun transformEventToCompiledEvent(event: Event): EventDescriptor =
        EventDescriptor(
            event.name(),
            event.providedIngredients.map(::ingredientToCompiledIngredient).asScala)
    ...
    .map { ingredient ->
        when (ingredient.name()) {
            com.ing.baker.recipe.common.`package$`.`MODULE$`.recipeInstanceIdName() -> com.ing.baker.il.`package$`.`MODULE$`.recipeInstanceIdName()
            com.ing.baker.recipe.common.`package$`.`MODULE$`.recipeInstanceMetadataName() -> com.ing.baker.il.`package$`.`MODULE$`.recipeInstanceMetadataName()
            com.ing.baker.recipe.common.`package$`.`MODULE$`.recipeInstanceEventListName() -> com.ing.baker.il.`package$`.`MODULE$`.recipeInstanceEventListName()
            else -> interactionDescriptor.overriddenIngredientNames().getOrElse(ingredient.name(), {ingredient.name()})
        } to ingredient.ingredientType()
    }.asScala

    val originalEvents = interactionDescriptor.output.map(::transformEventToCompiledEvent).asScala
    val eventsToFire = interactionDescriptor.output.map(::transformEventType).map(::transformEventToCompiledEvent).asScala
    ...
    val predefinedIngredientsWithOptionalsEmpty: Map<String, Value> =
        inputFields.asJava
            ...
            .map { Pair(it.first, `NullValue$`.`MODULE$`) }.toMap() +
                interactionDescriptor.predefinedIngredients

    val p: Pair<com.ing.baker.il.failurestrategy.InteractionFailureStrategy, Option<EventDescriptor>> =
        when (val strategy = interactionDescriptor.failureStrategy().getOrElse {defaultFailureStrategy }) {
            is InteractionFailureStrategy.RetryWithIncrementalBackoff -> {
                val exhaustedRetryEvent = when (val e = strategy.fireRetryExhaustedEvent()) {
                    is Some -> Some(EventDescriptor(
                        e.value().getOrElse { null as String? } ?: (interactionDescriptor.name() + com.ing.baker.il.`package$`.`MODULE$`.exhaustedEventAppend()), emptyList<IngredientDescriptor>().asScala)
                    )
                    else -> Option.empty()
                }
                RetryWithIncrementalBackoff(strategy.initialDelay(), strategy.backoffFactor(), strategy.maximumRetries(), strategy.maxTimeBetweenRetries(), exhaustedRetryEvent) to exhaustedRetryEvent
            }
            is InteractionFailureStrategy.BlockInteraction -> com.ing.baker.il.failurestrategy.`BlockInteraction$`.`MODULE$` to Option.empty()
            is InteractionFailureStrategy.FireEventAfterFailure -> {
                val eventName = strategy.eventName().getOrElse { interactionDescriptor.name() + com.ing.baker.il.`package$`.`MODULE$`.exhaustedEventAppend() }
                val exhaustedRetryEvent: EventDescriptor = EventDescriptor(eventName, emptyList<IngredientDescriptor>().asScala)
                com.ing.baker.il.failurestrategy.FireEventAfterFailure(exhaustedRetryEvent) to Some(exhaustedRetryEvent)
            }
           else -> com.ing.baker.il.failurestrategy.`BlockInteraction$`.`MODULE$` to Option.empty()
        }
    val failureStrategy = p.first
    val exhaustedRetryEvent = p.second.toList().toSeq()

    return InteractionTransition(
        (eventsToFire.asJava + exhaustedRetryEvent.asJava).asScala,
        (originalEvents.asJava + exhaustedRetryEvent.asJava).asScala,
...
private fun interaction(
    ...
    output: List<Event>,
    requiredEvents: kotlin.collections.Set<String> = emptySet(),
    requiredOneOfEvents: kotlin.collections.Set<kotlin.collections.Set<String>> = emptySet(),
    ...
    com.ing.baker.recipe.scaladsl.Interaction.apply(
        name,
        inputIngredients.asScala,
        output.asScala,
        requiredEvents.asScala,
        requiredOneOfEvents.map { it: kotlin.collections.Set<String> -> it.asScala}.toSet().asScala,
...
val InteractionTransition.maximumInteractionCount get(): Int? = this.maximumInteractionCount().getOrElse { null as Int? }
...
val EventOutputTransformer.newEventName get() = this.newEventName()
