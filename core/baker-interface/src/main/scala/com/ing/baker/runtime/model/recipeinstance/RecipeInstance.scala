...
case class RecipeInstance(
  ...
  /** Validates an attempt to fire an event, and if valid it suspends the effect of such action.
    ...
    */
  def fire[F[_]](input: EventInstance, correlationId: Option[String] = None)(implicit components: BakerComponents[F], effect: Sync[F], clock: Clock[F]): FireOutcome[F, (RecipeInstance, F[TransitionExecutionOutcome])] =
    for {
      firing <- reportRejectionReason(validateInputAndCreateExecution(input, correlationId))
...
object RecipeInstance {
  ...
  type FireOutcome[F[_], A] =
    EitherT[F, FireSensoryEventRejection, A]
