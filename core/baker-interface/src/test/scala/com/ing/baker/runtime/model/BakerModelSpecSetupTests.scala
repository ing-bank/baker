...
trait BakerModelSpecSetupTests {
  ...
  def runSetupTests()(implicit sync: Sync[IO], async: Async[IO], classTag: ClassTag[IO[Any]]): Unit = {
    ...
    test("providing implementations in a sequence") { context =>
      for {
        _ <- context.setupBakerWithNoRecipe(mockImplementations)
      } yield succeed
    }

    test("providing an implementation with the class simple name same as the interaction") { context =>
      for {
        _ <- context.setupBakerWithNoRecipe(mockImplementations)
      } yield succeed
    }
    ...
    test("throw a exception when an invalid recipe is given") { context =>
      ...
      for {
        ...
        _ <- baker.addRecipe(RecipeRecord.of(RecipeCompiler.compileRecipe(recipe))).attempt.map {
          case Left(e) => e should have(Symbol("message")("Recipe NonProvidedIngredient:68b775e508fc6877 has validation errors: Ingredient 'initialIngredient' for interaction 'InteractionOne' is not provided by any event or interaction"))
    ...
    test("throw a exception when a recipe does not provide an implementation for an interaction") { context =>
      ...
      for {
        ...
        _ <- baker.addRecipe(RecipeRecord.of(RecipeCompiler.compileRecipe(recipe))).attempt.map {
          case Left(e) => e should have(Symbol("message")("Recipe MissingImplementation:dc3970efc8837e64 has implementation errors: No compatible implementation provided for interaction: InteractionOne: List(NameNotFound)"))
          case Right(_) => fail("Adding a recipe should fail")
        }
      } yield succeed
    }
    ...
    test("throw a exception when a recipe provides an implementation for an interaction and does not comply to the Interaction") { context =>
      ...
      for {
        ...
        _ <- baker.addRecipe(RecipeRecord.of(RecipeCompiler.compileRecipe(recipe))).attempt.map {
          case Left(e) => e should have(Symbol("message")("Recipe WrongImplementation:8e2745de0bb0bde5 has implementation errors: No compatible implementation provided for interaction: InteractionOne: List(InteractionOne input size differs: transition expects 2, implementation provides 1)"))
