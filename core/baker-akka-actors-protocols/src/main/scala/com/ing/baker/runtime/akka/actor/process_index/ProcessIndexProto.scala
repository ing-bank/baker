...
import com.ing.baker.runtime.akka.actor.process_index.ProcessIndexProtocol.{ActorActivated, ActorCreated, ActorDeleted, ActorMetadata, ActorPassivated, GetShardIndex, ProcessIndexSnapShot}
    ...
    new ProtoMap[ActorMetadata, protobuf.ActorMetaData] {
      ...
      def toProto(a: ActorMetadata): protobuf.ActorMetaData =
        protobuf.ActorMetaData(
          ...
          Some(a.processStatus == Deleted),
          Some(a.processStatus == Passivated)
        )
        ...
        for {
          entries <- message.entries.toList.traverse[Try, ActorMetadata] { e =>
            for {
              ...
              processStatus = if (isDeleted) Deleted else Active
            } yield ActorMetadata(recipeId, recipeInstanceId, createdDateTime, processStatus)
          }
        } yield Index(entries)
    }
