...
case class SRecipe(override val name: String,
  ...
  def withEvents(eventClasses: Class[_]*): SRecipe = copy(events = eventClasses.toSet)
...
object SRecipe {
  ...
  def apply(name: String, interactions: Seq[InteractionDescriptor[_]], events: Set[Class[_]]) : SRecipe =
