...
class HealthServiceSpec  extends BakeryFunSpec with Matchers {
  ...
  def contextBuilder(testArguments: TestArguments): Resource[IO, TestContext] = {
    ...
    for {
      server <- HealthService.resource(address)
  ...
  test("Health endpoint") { context =>
    context.client.status(GET(Uri.unsafeFromString(s"http://localhost:${context.port}") / "health")).map(status => assert(status.code == 200))
  }
}
