...
object RemoteInteractionClient {
   ...
   */
  def resource(uri: Uri, pool: ExecutionContext, tlsConfig: Option[BakeryHttp.TLSConfig])(implicit timer: Temporal[IO]): Resource[IO, RemoteInteractionClient] =
...
final class RemoteInteractionClient(client: Client[IO], uri: Uri)(implicit cs: ContextShift[IO], timer: Temporal[IO]) {
