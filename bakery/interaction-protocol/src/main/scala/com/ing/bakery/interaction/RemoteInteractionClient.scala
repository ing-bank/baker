...
object RemoteInteractionClient {
  ...
  def resource(headers: Headers,
               ...
               tlsConfig: Option[BakeryHttp.TLSConfig]): Resource[IO, RemoteInteractionClient] = {
    ...
    client.expect[ExecutionResult](
      ...
      .flatMap {
        case InteractionExecution.ExecutionResult(Right(success)) =>
          IO {
            metricService.interactionSuccessCounter.inc()
            success.result
          }
        case InteractionExecution.ExecutionResult(Left(error)) =>
          IO.raiseError {
            metricService.interactionFailureCounter.inc()
